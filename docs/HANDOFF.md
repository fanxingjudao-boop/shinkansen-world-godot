...
- **コミット状況**: 本番 `origin/main` は **v0.52.0(`42a4ccc`)まで push 済・working tree クリーン・未 push なし**(改善さんが都度 push 済)。このセッションの実装(すべて push 済): **v0.51.0 A-5乗客 → v0.51.1 腕削除 → v0.51.2 全機能監査+月・空の戻る常時表示 → v0.52.0 B-5楽器**。
- **🎯 次セッションの最重要 = 改善さんの実機/ブラウザ体感確認**(v0.50.0〜v0.52.0 の新要素)。OK なら **Web 再エクスポート → push**(`export/web`、下記デプロイ手順)。新規実装を続けるなら候補は `docs/PLAYFUL_DETAILS.md` の **A-2 動物のしぐさ / B-7 かくれた でんしゃ / C-3 季節と天気** ほか(いずれも `_spawn_extra` 寄生方式で追加可)。
  ...
  - **⚠️ Web 再エクスポートはまだ。** v0.50.0〜v0.52.0 を実機/ブラウザで遊ぶには `export/web` の再エクスポートが必要(下記デプロイ手順)。改善さんがソース実行(F5)で先に体感確認してもよい。ソースは push 済だが **ライブサイトは再エクスポートまで旧ビルド(v0.49.0)のまま**。
