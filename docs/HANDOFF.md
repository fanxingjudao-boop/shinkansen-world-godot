...
- **🆕 v0.58.0 = スマホ画面で操作しやすく(2026-06-07)**(改善さんリクエスト): ①UIを大きく=基準解像度 `1024×576→896×504`(`project.godot`・stretch viewport なので全UI約14%拡大)②D-pad を 72→80px・コンテナ256・矢印font44(`TouchHUD.tscn`)③ノッチ対策=body に `env(safe-area-inset-*)` 余白(`web/template.html`)④たて持ちで「よこむきに してね」オーバーレイ(`web/template.html`・portrait のみ)。**Web 再エクスポート済**(index.html+index.pck 更新・for_mobile=false/index.*/noindex 維持確認)。**要 push + 改善さんの実機確認**: ボタンの大きさ・iPad でUIが大きすぎないか・ノッチ機種・たて持ち案内。**調整候補**: 解像度(`project.godot` viewport_width/height)・D-pad 寸法・案内文言。
  - **🎯 次セッション最初 = 改善さんに `! git push origin main` を依頼 → スマホ/iPad で体感確認**。
- **✅ 一つ前(2026-06-07)**: `d05292b` コード掃除(挙動不変)+ `f4c55cb` 引き継ぎ更新まで push 済み。ライブは v0.57.0(主人公えらび)+ 掃除。
...
- **コミット状況(2026-06-07 更新)**: `origin/main` = ローカル = **`d05292b`**(コード掃除)で完全同期・push 済み。その手前に `5e9fe53`(v0.57.0 主人公えらび + Web 再エクスポート)。**未 push なし・作業ツリー クリーン**。**export/web は v0.57.0 を含めて再エクスポート済**(コード掃除はロジック不変なので再エクスポート不要)。次に Web を再エクスポートするのは「ゲーム挙動を変える新実装」をしたとき。
