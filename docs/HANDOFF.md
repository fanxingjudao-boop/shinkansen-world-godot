...
- **🆕 v0.59.0 = えほんのき(2026-07-03・Web 再エクスポート済・未 push)**: 寓話(ファブル)が実る木。草原(-16,14)に絵本の木+近接「えほんを よむ」→ 絵本オーバーレイで 3 ページの小さなおとぎばなし(6 話・ひらがなのみ・やさしい結末)。新規 `scripts/world/story_tree.gd`(`_spawn_extra`・Main.tscn 不変・セーブなし)。`AUTO_STORY` で検証済。**要 改善さんの確認**: 文章・木の見た目・文字サイズ・場所・音量。おはなしの追加は `STORIES` 配列に足すだけ。
